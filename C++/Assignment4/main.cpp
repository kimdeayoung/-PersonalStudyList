...
void DuplicateDeleteTest()
{
	const int checkArrSize = 5;
	BinarySearchTree<int> tree;
	int arr[checkArrSize] = { 15, 10, 14, 15, 19 };
	...
	int checkArr1[checkArrSize] = { 10, 14, 15, 15, 19 };
	for (int i = 0; i < checkArrSize; ++i)
	{
		assert(v[i] == checkArr1[i]);
	}

	assert(tree.Delete(15));
	int checkArr2[checkArrSize - 1] = { 10, 14, 15, 19 };
	v = tree.TraverseInOrder(tree.GetRootNode().lock());
	for (int i = 0; i < checkArrSize - 1; ++i)
	{
		assert(v[i] == checkArr2[i]);
	}

	assert(tree.Delete(15));
	int checkArr3[checkArrSize - 2] = { 10, 14, 19 };
	v = tree.TraverseInOrder(tree.GetRootNode().lock());
	for (int i = 0; i < checkArrSize - 2; ++i)
	{
		assert(v[i] == checkArr3[i]);
	}
}
