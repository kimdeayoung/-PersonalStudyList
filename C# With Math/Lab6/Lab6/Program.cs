...
namespace Lab6
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            item1 = new Item(EType.Furniture, 1.99999, 1.245, true);
            item2 = new Item(EType.Paper, 1.99999, 10.568, true);
            item3 = new Item(EType.Furniture, 0.8752, 11.23, false);
            item4 = new Item(EType.Paper, 5.234, 16.9, false);
            item5 = new Item(EType.Electronics, 5.00001, 11, true);
            item6 = new Item(EType.Paper, 6.121, 15, true);
            item7 = new Item(EType.Electronics, 6.121, 10, false);
            item8 = new Item(EType.Paper, 7.562, 15, false);

            List<Item> moreTestItems = new List<Item>
            {
                item1,
                item2,
                item3,
            ...
            foreach (Item item in moreTestItems)
            {
                bot.Add(item);
            }
            List<Item> moreTestDumps = bot.Dump();

            List<Item> moreTestExpectedDumps = new List<Item>
            {
                item1,
                item3,
                item4,
            ...
            Debug.Assert(moreTestDumps.Count == moreTestExpectedDumps.Count);

            for (int i = 0; i < moreTestExpectedDumps.Count; i++)
            {
                Debug.Assert(itemEquals(moreTestDumps[i], moreTestExpectedDumps[i]));
            }
        }
